...
export function useFilters() {
  ...
  const filters: Filters = useMemo(() => {
    ...
    const rawIsValid =
      rawPreset === 'day' || rawPreset === 'week' || rawPreset === 'month' || rawPreset === 'custom'
  ...
  const setPreset = useCallback(
    (preset: Exclude<Preset, 'custom'>) => patch({ preset, from: null, to: null }),
    [patch],
  )
  const setCustomRange = useCallback(
    (range: DateRange) => patch({ preset: 'custom', from: range.from, to: range.to }),
    [patch],
  )
  ...
  const reset = useCallback(
    () => patch({ preset: null, from: null, to: null, car: null, driver: null }),
    [patch],
  )
