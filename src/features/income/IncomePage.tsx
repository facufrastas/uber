...
export function IncomePage() {
  ...
  const paymentByShift = useMemo(
    () => new Map(data.payments.map((p) => [p.shiftId, p])),
    [data.payments],
  )
  ...
  const sortedShifts = useMemo(
    () => [...filtered.shifts].sort((a, b) => b.date.localeCompare(a.date)),
    [filtered.shifts],
  )
    ...
    <>
      ...
      {sortedShifts.length === 0 ? (
        <EmptyState
          ...
          description="Ajustá los filtros o registrá un turno nuevo."
        />
      ) : (
                ...
                return (
                  <TableRow key={shift.id}>
                    ...
                    <TableCell className="font-medium">
                      {driverById(shift.driverId)?.name ?? '—'}
                    </TableCell>
                    <TableCell>
                      {car ? (
                    ...
                    <TableCell className="text-right font-medium tabular-nums">
                      {payment ? formatARS(payment.amount) : '—'}
                    </TableCell>
                    <TableCell>
                      <div className="flex justify-end gap-1">
                        <Button
                          ...
                          size="icon-sm"
                          aria-label="Editar"
                          onClick={() => setDialog({ open: true, shift, payment })}
                        >
                          <Pencil />
                        </Button>
                        <ConfirmDeleteDialog
