...
export function MaintenancePage() {
  ...
  const expenseByMaintenance = useMemo(
    () => new Map(data.expenses.filter((e) => e.maintenanceId).map((e) => [e.maintenanceId!, e])),
    [data.expenses],
  )
  ...
  const visible = useMemo(
    () =>
      maintenances
        .filter(
          (m) =>
            m.date >= filters.range.from &&
            m.date <= filters.range.to &&
            (!filters.carId || m.carId === filters.carId),
        )
  ...
  return (
    <>
      <PageHeader
        ...
        description="Historial de servicio de cada auto; el costo se refleja en Gastos"
      >
      ...
      {visible.length === 0 ? (
        <EmptyState
          icon={Wrench}
          title="Sin mantenimientos en el período"
          description="Ajustá los filtros o registrá un mantenimiento."
        />
      ) : (
                ...
                return (
                  <TableRow key={maintenance.id}>
                    ...
                    <TableCell className="text-right tabular-nums text-muted-foreground">
                      {maintenance.km ? maintenance.km.toLocaleString('es-AR') : '—'}
                    </TableCell>
                    <TableCell className="text-right font-medium tabular-nums">
                      {expense ? formatARS(expense.amount) : '—'}
                    </TableCell>
                    <TableCell>
                      <div className="flex justify-end gap-1">
                        <Button
                          ...
                          size="icon-sm"
                          aria-label="Editar"
                          onClick={() => setDialog({ open: true, maintenance, expense })}
                        >
                          <Pencil />
                        </Button>
                        <ConfirmDeleteDialog
