...
export function applyFilters(data: DataSlice, filters: Filters): FilteredData {
  ...
  const driversCarId = driverId
    ? (data.drivers.find((d) => d.id === driverId)?.carId ?? null)
    : null
  const expenseCarId = carId ?? driversCarId

  const expenses = data.expenses.filter(
    (e) => inRange(e.date, range) && (!expenseCarId || e.carId === expenseCarId),
  )
...
// doesn't "jump".
export function incomeVsExpensesByDay(
  data: DataSlice,
  ...
  const byDay = new Map<string, DailyPoint>(
    daysInRange(range).map((date) => [date, { date, income: 0, expenses: 0 }]),
  )
...
export function incomeByCar(data: DataSlice, filtered: FilteredData): AmountByName[] {
  ...
  const sums = sumBy(filtered.payments, (p) => shiftCar.get(p.shiftId) ?? null, (p) => p.amount)
  return data.cars
...
export function expensesByCar(data: DataSlice, filtered: FilteredData): AmountByName[] {
  const sums = sumBy(filtered.expenses, (e) => e.carId, (e) => e.amount)
  return data.cars
    .map((c) => ({ id: c.id, name: `${c.model} ${c.licensePlate}`, amount: sums.get(c.id) ?? 0 }))
    .filter((r) => r.amount > 0)
    .sort((a, b) => b.amount - a.amount)
...
export function incomeByDriver(data: DataSlice, filtered: FilteredData): AmountByName[] {
  ...
  const sums = sumBy(filtered.payments, (p) => shiftDriver.get(p.shiftId) ?? null, (p) => p.amount)
  return data.drivers
...
export function expensesByType(data: DataSlice, filtered: FilteredData): AmountByName[] {
  const sums = sumBy(filtered.expenses, (e) => e.expenseTypeId, (e) => e.amount)
